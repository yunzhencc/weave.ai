...
import { getCatalog, saveProvider, saveModel, saveBinding } from '../models/server/db/catalog.ts'
...
const call = (path: string, value?: unknown) => handleAiRequest(new Request(`http://localhost/api/ai/${path}`, {
  method: value === undefined ? 'GET' : 'POST', headers: path.startsWith('admin/') ? {} : { authorization: 'Bearer runtime-test' },
...
test('configuration API works without auth and a dynamic text binding runs through TanStack', async () => {
  const cleanup = setup()
  try {
...
test('task retries and video polling retain original binding and credential after rotation and disable', async () => {
  const cleanup = setup()
  try {
...
test('legacy stored tasks and concurrent repeated submissions preserve idempotency', async () => {
  const cleanup = setup()
  try {
    ...
    fetchMock.mockImplementation(async (url, init) => {
      const req = new Request(url, init)
      if (req.method === 'POST') return Response.json({ request_id: 'job', status: 'IN_QUEUE' })
 ...
 test('bodyless provider checks accept an empty transport stream', async () => {
  const cleanup = setup()
  try {

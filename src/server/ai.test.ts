...
import { readGeneration, reserveGeneration, updateGeneration } from '../models/server/db/generated-assets.ts';
...
it('model integration validates inputs, protects paid calls and keeps durable idempotency', async () => {
...
it('openRouter streams text and never exposes upstream error details', async () => {
  vi.stubEnv('AI_API_TOKEN', 'test-secret');
  ...
  const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (fail)
      return Response.json({ error: { message: 'private-provider-detail', code: 400 } }, { status: 400 });
...
it('real fal adapters submit once, map model parameters, poll saved jobs and persist results', async () => {
  ...
  const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (req.method === 'POST') {
      if (fail)
        return Response.json({ error: 'secret upstream information' }, { status: 503 });
      return Response.json({ request_id: 'provider-123', status: 'IN_QUEUE' });
    }
    if (req.url.includes('/status'))
      return Response.json({ status: 'COMPLETED' });
    if (resultUnavailable)
      return Response.json({ detail: 'temporary result access error' }, { status: 401 });
    if (req.url.includes('/flux/') || req.url.includes('/gpt-image-2.5/'))
      return Response.json({ images: [{ url: 'https://example.com/image.png', width: 1024, height: 1024, content_type: 'image/png' }] });

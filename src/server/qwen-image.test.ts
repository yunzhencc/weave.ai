...
it('qwen image models map text and reference images to native DashScope requests and persist idempotent results', async () => {
  ...
  try {
    ...
    for (const referenceImages of ['https://reference.example/a.png', [123], ['data:image/png;base64,AA=='], ['file:///tmp/image.png'], ['https://user:password@example.com/image.png'], ['invalid'], Array.from({ length: 4 }).fill('https://reference.example/a.png')]) {
      const id = randomUUID();
...
it('uncertain DashScope image outcomes are sanitized and never resubmitted', async () => {

...
import { generationIdentity, parseGeneration, parseGenerationIntent } from '../../models/generation.ts';
...
import { pollStudioVideoJob, submitStudioVideoJob } from './studio-video-generation.ts';
...
export async function generate(input: Generation) {
  ...
  if (existing) {
    if (generationIdentity(existing) !== generationIdentity(input))
...
export async function poll(id: string) {
  ...
  if (record.kind !== 'videos' || !record.providerJobId || !['pending', 'processing'].includes(record.status))
    return record;
...
export async function generateRequest(kind: 'images' | 'videos', value: unknown) {
  ...
  if (existing) {
    if (generationIdentity(existing) !== generationIdentity(intent))
      throw new ModelError('conflict', 'This id belongs to a different request');
    return existing;
  }

...
export function createStorage(env: NodeJS.ProcessEnv = process.env): Disk {
  ...
  if (driver === 'fs') return new Disk(new FSDriver({ visibility: 'private', location: resolve(env.STORAGE_FS_ROOT || '.data/uploads') }))
  if (driver !== 's3') throw new Error('STORAGE_DRIVER must be fs or s3')
  ...
  if (endpoint && !['https:', 'http:'].includes(new URL(endpoint).protocol)) throw new Error('Invalid STORAGE_S3_ENDPOINT')

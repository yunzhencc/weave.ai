...
test('文件接口鉴权、二进制读写、大小限制及本地持久化', async () => {
  ...
  const request = (method: string, body?: Uint8Array) => new Request('http://localhost/api/ai/files', {
    method, headers: { Authorization: 'Bearer storage-test' }, body: body ? new Uint8Array(body) : undefined,
  })
  try {
...
test('S3 使用自定义 endpoint 签名，配置错误不回退到本地', async () => {
  const env = {
    STORAGE_DRIVER: 's3', STORAGE_S3_REGION: 'us-east-1', STORAGE_S3_BUCKET: 'test-bucket',
    STORAGE_S3_ENDPOINT: 'https://storage.example.com', STORAGE_S3_ACCESS_KEY_ID: 'test-key',

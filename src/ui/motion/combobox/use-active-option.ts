 ...
 * highlight.
 */
interface ActiveCursor { value: string; query: string }
...
function isEnabled(enabledItems: Options['enabledItems'], candidate: string | undefined): candidate is string {
  return candidate !== undefined && enabledItems.some(i => i.value === candidate);
}
...
function resolveActive(cursor: ActiveCursor | null, options: Options) {
  return liveCursorValue(cursor, options) ?? fallbackActive(options);
}

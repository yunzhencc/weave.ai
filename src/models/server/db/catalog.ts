import type { AdapterId, Binding, BindingInput, Candidate, Catalog, ExecutionSnapshot, Model, ModelFilter, ModelInput, ModelKind, Provider, ProviderInput, ProviderType } from '../../catalog.ts';
...
import { createCredential, decryptCredential } from '../credentials.ts';
...
function parse<T>(schema: z.ZodType<T>, value: unknown): T {
  ...
  if (!result.success)
    throw new ModelError('invalid_input', 'Invalid model configuration');
  return result.data;
}
...
function initialize(db: DatabaseSync) {
  ...
  if (db.prepare('SELECT version FROM model_migrations WHERE version = 1').get())
    return;
...
export function saveProvider(input: ProviderInput): Provider {
  ...
  if ((data.type === 'fal' && baseUrl !== 'https://fal.run') || (data.type === 'openrouter' && baseUrl !== 'https://openrouter.ai/api/v1'))
    throw new ModelError('invalid_input', 'This provider uses a fixed URL');
  return database((db) => {
    const id = data.id ?? randomUUID();
    const previous = read<StoredProvider>(db, 'model_providers', id);
    const next = nextRevision(previous, data.revision);
    if (previous && previous.type !== data.type && all<Binding>(db, 'model_bindings').some(b => b.providerId === id && !adapters[data.type].includes(b.adapter)))
      throw new ModelError('conflict', 'Provider type is incompatible with existing bindings');
    const replacing = data.apiKey !== undefined || data.envKey !== undefined;
    if (previous?.credentialId && new URL(previous.baseUrl).origin !== url.origin && !replacing && !data.confirmCredentialReuse)
      throw new ModelError('conflict', 'Confirm credential reuse when changing provider origin');
...
function saveModelInDb(db: DatabaseSync, input: ModelInput): Model {
  ...
  const next = nextRevision(previous, data.revision);
  if (data.defaultBindingId && read<Binding>(db, 'model_bindings', data.defaultBindingId)?.modelId !== data.id)
    throw new ModelError('invalid_input', 'Default binding must belong to this model');
  if (all<Binding>(db, 'model_bindings').some(b => b.modelId === data.id && adapterKinds[b.adapter] !== data.kind))
    throw new ModelError('conflict', 'Model kind is incompatible with existing bindings');
...
function saveBindingInDb(db: DatabaseSync, input: BindingInput): Binding {
  ...
  if (!model || !provider || adapterKinds[data.adapter] !== model.kind || !adapters[provider.type].includes(data.adapter))
    throw new ModelError('invalid_input', 'Incompatible model, provider, or adapter');
  if (previous && previous.modelId !== data.modelId)
    throw new ModelError('conflict', 'Binding model cannot be changed');
  const cap = data.capabilities;
  if (model.kind === 'text' ? Object.keys(cap).length > 0 : !cap.aspectRatios || (model.kind === 'images' ? cap.maxReferenceImages === undefined || cap.durations !== undefined : !cap.durations || cap.maxReferenceImages !== undefined))
    throw new ModelError('invalid_input', 'Capabilities do not match adapter');
  if (model.kind !== 'text') {
    const template = Object.values(seeds).find(m => m.endpoint === data.upstreamModelId && m.kind === model.kind && m.via === provider.type);
    if (!template || ('maxReferenceImages' in template && cap.maxReferenceImages! > template.maxReferenceImages))
      throw new ModelError('invalid_input', 'Unsupported media endpoint or capabilities');
  }
  if (all<Binding>(db, 'model_bindings').some(b => b.id !== data.id && b.modelId === data.modelId && b.providerId === data.providerId && b.adapter === data.adapter && b.upstreamModelId === data.upstreamModelId))
    throw new ModelError('conflict', 'Binding already exists');
...
export function resolveModel(input: { modelId: string; bindingId?: string; kind: ModelKind }): ExecutionSnapshot {
  ...
  if (!model || model.kind !== input.kind)
    throw new ModelError('invalid_input', 'Unsupported model capability');
  ...
  if (!model.enabled || !binding?.enabled || !provider?.enabled || !provider.configured || !provider.credentialId)
...
export function importCandidate(providerId: string, candidate: Candidate, modelId: string, enabled = false): Catalog {
  database((db) => {
    ...
    if (!model)
      model = saveModelInDb(db, { id: modelId, name: candidate.name, vendor: candidate.vendor, kind: candidate.kind, enabled: true, defaultBindingId: null });
    if (model.kind !== candidate.kind)
      throw new ModelError('invalid_input', 'Candidate capability does not match model');
    ...
    if (!model.defaultBindingId)
      saveModelInDb(db, { ...model, defaultBindingId: binding.id });
  });
  return getCatalog();
}

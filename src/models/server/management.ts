...
import { getCatalog, importCandidate, saveBinding, saveModel, saveProvider } from './db/catalog.ts';
...
export async function manageModels(path: string, method: string, input?: unknown) {
  ...
  if (path === 'providers') {
    ...
    if (!data.success)
      throw new ModelError('invalid_input', 'Invalid provider configuration');
    await validateProviderUrl(data.data.baseUrl);
    return saveProvider(input as ProviderInput);
  }
  if (path === 'models')
    return saveModel(input as ModelInput);
  if (path === 'bindings')
    return saveBinding(input as BindingInput);
  if (path === 'import') {
    ...
    if (!data.success)
      throw new ModelError('invalid_input', 'Invalid import request');
  ...
  if (operation) {
    ...
    if (!provider)
      throw new ModelError('not_found', 'Channel not found');
    if (!provider.credentialId)
      throw new ModelError('not_configured', 'Channel has no credential');
...
export function publicModels() {
  ...
  return catalog.models.map((model) => {
    ...
    return { id: model.id, name: model.name, kind: model.kind, vendor: model.vendor, via, endpoint: binding?.upstreamModelId ?? null, ...binding?.capabilities, configured: provider?.configured ?? false, available: Boolean(model.enabled && binding?.enabled && provider?.enabled && provider.configured), bindings: catalog.bindings.filter(b => b.modelId === model.id).map((b) => {

...
export type Credential = { id: string; source: 'env' | 'encrypted'; envKey: string | null; ciphertext: string | null; iv: string | null; tag: string | null; hint: string | null }

function encryptionKey() {
  const value = process.env.MODEL_ENCRYPTION_KEY?.trim() ?? ''
  const key = Buffer.from(value, 'base64')
  if (key.length !== 32 || key.toString('base64') !== value) throw new ModelError('not_configured', 'MODEL_ENCRYPTION_KEY must be a base64 encoded 32-byte key')
  return key
}

export function createCredential(input: { apiKey?: string; envKey?: string }): Credential {
  if (input.apiKey !== undefined && input.envKey !== undefined) throw new ModelError('invalid_input', 'Choose one credential source')
  const base = { id: randomUUID(), ciphertext: null, iv: null, tag: null, hint: null }
  if (input.envKey !== undefined) {
    if (!credentialEnvironmentNames.some((name) => name === input.envKey)) throw new ModelError('invalid_input', 'Unsupported credential environment variable')
    return { ...base, source: 'env', envKey: input.envKey }
  }
  const secret = input.apiKey?.trim()
  if (!secret) throw new ModelError('invalid_input', 'API key must not be empty')
...
export function decryptCredential(record: Credential): string {
  if (record.source === 'env') {
    if (!credentialEnvironmentNames.some((name) => name === record.envKey)) throw new ModelError('not_configured', 'Unsupported credential reference')
    const secret = process.env[record.envKey!]?.trim()
    if (!secret) throw new ModelError('not_configured', 'Provider credential is not configured')
    return secret
  }
  try {
...
export function resolveCredential(id: string): string {
  ...
  if (!credential) throw new ModelError('not_configured', 'Provider credential is missing')

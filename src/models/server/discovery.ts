...
async function readDirectory(provider: Provider, apiKey: string, path: string): Promise<unknown> {
  if (!apiKey.trim()) throw new ModelError('not_configured', '请先配置渠道凭据')
  try {
    ...
    try {
      while (true) {
        ...
        if (size > 2 * 1024 * 1024) throw new Error('directory too large')
        chunks.push(value)
      }
...
export async function discoverModels(provider: Provider, apiKey: string): Promise<{ candidates: Candidate[]; checkedAt: string }> {
  if (provider.type === 'fal') {
    const candidates: Candidate[] = Object.entries(models).flatMap(([name, model]) => {
      if (model.via !== 'fal') return []
      return [{ upstreamModelId: model.endpoint, name, vendor: model.vendor, kind: model.kind,
        adapter: model.kind === 'images' ? 'fal-image' : 'fal-video', source: 'builtin',
        capabilities: { aspectRatios: [...model.aspectRatios], ...('durations' in model ? { durations: [...model.durations] } : { maxReferenceImages: model.maxReferenceImages }) },
      }]
    })
    return { candidates, checkedAt: new Date().toISOString() }
  }
  const result = await readDirectory(provider, apiKey, 'models')
  if (!record(result) || !Array.isArray(result.data)) throw new ModelError('provider_unknown', '渠道未返回支持的模型列表；请手动配置，未切换备用来源')
  const candidates: Candidate[] = []
  const seen = new Set<string>()
  for (const item of result.data) {
    if (!record(item) || typeof item.id !== 'string' || !item.id.trim() || item.id.length > 256 || seen.has(item.id)) continue
    ...
    if (!textMetadata && !(provider.type !== 'openrouter' && !architecture && knownText)) continue
    seen.add(item.id)
    candidates.push({ upstreamModelId: item.id, name: typeof item.name === 'string' ? item.name.slice(0, 200) : item.id,
      vendor: provider.type === 'dashscope' ? 'Alibaba' : item.id.includes('/') ? item.id.split('/')[0] : 'Unknown',
      kind: 'text', adapter: provider.type === 'openrouter' ? 'openrouter-text' : 'openai-compatible-text', source: 'remote', capabilities: {},
    })
  }
  return { candidates, checkedAt: new Date().toISOString() }
}
...
export async function checkProvider(provider: Provider, apiKey: string): Promise<{ configured: boolean; message: string }> {
  if (!apiKey.trim()) return { configured: false, message: '尚未配置渠道凭据' }
  if (provider.type === 'fal') return { configured: true, message: '仅检查配置，未验证供应商凭据' }

...
import { parseGeneration, type GenerationRecord } from '../generation.ts'
...
it('isolates credentials through concurrent image, video submission and video polling awaits', async () => {
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    ...
    const job = request.method === 'POST' ? (await request.json()).prompt as string
    ...
    if (request.method === 'POST') return Response.json({ request_id: `${job}-job`, status: 'IN_QUEUE' })

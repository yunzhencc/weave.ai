...
function parsedUrl(value: string) {
  ...
  if (url.username || url.password || url.hash || url.search || !hostname || (url.protocol !== 'https:' && !(allowed && url.protocol === 'http:'))) invalid()
  if (!allowed && (hostname === 'localhost' || hostname.endsWith('.localhost'))) invalid()
...
async function resolveUrl(value: string) {
  ...
  if (!target.allowed && addresses.some(({ address, family }) => family === 4
    ...
    : !globalV6.check(address, 'ipv6') || blocked.check(address, 'ipv6'))) invalid()
...
export function providerFetch(baseUrl: string): typeof fetch {
  ...
  return async (input, init) => {
    ...
    if (url.origin !== base.origin || url.username || url.password || url.hash) invalid()

...
export function parseGeneration(kind: 'images' | 'videos', value: unknown, selected?: { kind: ModelKind; via: Generation['via']; endpoint: string } & Capabilities): Generation {
  ...
  if (typeof input.model !== 'string' || !input.model.trim()) throw new ModelError('invalid_input', 'Invalid model')
  if (input.bindingId !== undefined && (typeof input.bindingId !== 'string' || !input.bindingId.trim())) throw new ModelError('invalid_input', 'Invalid binding')
  ...
  if (!Array.isArray(references) || references.length > (model.kind === 'images' ? model.maxReferenceImages ?? 0 : 0)) throw new ModelError('invalid_input', 'Invalid referenceImages count for this model')
  const referenceImages = references.map((value: unknown) => {
    if (typeof value !== 'string') throw new ModelError('invalid_input', 'Invalid reference image URL')
    let url: URL
    try { url = new URL(value) } catch { throw new ModelError('invalid_input', 'Invalid reference image URL') }
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) throw new ModelError('invalid_input', 'Reference images must use HTTP(S) URLs without credentials')
    return url.href
  })
...
export function generationIdentity(input: Generation) {
  return JSON.stringify({ id: input.id, kind: input.kind, model: input.model, bindingId: input.bindingId ?? null,
    prompt: input.prompt, aspectRatio: input.aspectRatio, duration: input.duration ?? null,
...
export function parseText(value: unknown, dynamic = false) {
  ...
  if (typeof input.model !== 'string' || !input.model.trim()) throw new ModelError('invalid_input', 'Invalid model')
  if (input.bindingId !== undefined && (typeof input.bindingId !== 'string' || !input.bindingId.trim())) throw new ModelError('invalid_input', 'Invalid binding')
  if (!dynamic) modelFor('text', input.model)
